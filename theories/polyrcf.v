...
apply/negP => /andP[].
have := sorted_lt_nth ler_trans.
move=> 
rewrite sorted_lt_nth //= -?topredE /= ?size_rcons ?ltnS ?find_size 1?leqW //=.

  rewrite /snext/=.
by move=> /andP[] /leq_ltn_trans h /h; rewrite ltnn.
Qed.
...
have nrp' : {in `](sinf rs' cp x), (smax rs' cp x)[, noroot p^`()}.
...
- move=> /(uniq_root _ nrp' _ _ rpx)->//; rewrite ?inE;
  by rewrite ?(itvP (findrPmax _ _)) // ?inE ?(itvP xcp) //.
- rewrite ltrNge (root_has_ivt _ nrp' _ rpx) //;
  by rewrite ?inE ?(itvP (findrPmax _ _)) ?lerr //.
Qed.
...
Lemma roots_opp p : roots (- p) = roots p.
Proof.
...
(* Proof. *)
(* move=> derp_neq0; have [ltab|leab] := (ltrP a b); last first. *)
(*   by left => x; rewrite in_nil itv_gte. *)
(* have leab := ltrW ltab; have [] := ltrP (p.[a] * p.[b]) 0. *)
(*   move=> /poly_ivtoo [] // x xab rpx; left. *)
...
Definition prev_root p := sinf (roots p) (cauchy_bound p).
Definition next_root p := ssup (roots p) (cauchy_bound p).

Lemma next_root0 x : next_root 0 x = cauchy_bound 0.
Proof. by rewrite /next_root /ssup roots0. Qed.
